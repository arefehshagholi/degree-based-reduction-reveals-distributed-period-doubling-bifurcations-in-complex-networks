...
L_all = zeros(N, run_time, 3);
L_all(:,1,1) = 0.1*rand(N,1); L_all(:,1,2) = 0.1*rand(N,1); L_all(:,1,3) = 0.1*rand(N,1);
for g_idx = 1:length(w_values)
    g_ch = w_values(g_idx); A_weighted = g_ch * A;
    for t = 2:run_time
        [L_all(:,t,1), L_all(:,t,2), L_all(:,t,3)] = main_func(L_all(:,t-1,1), L_all(:,t-1,2), L_all(:,t-1,3), A_weighted);
    end
